...
class CodeQL:
    ...
    def _server_resolve_ram(self, max_ram=0):
        ...
        return shell_command_to_string(
            self.codeql_cli + ["resolve", "ram"] + max_ram_arg + ["--"]).strip().split('\n')
    ...
    def _server_run_query_from_path(self, bqrs_path, query_path,
                                    quick_eval_pos=None,
                                    template_values=None):
        library_paths = self._resolve_library_paths(query_path)
        return self._server_request_run(bqrs_path, query_path, library_paths,
                                        quick_eval_pos=quick_eval_pos,
    ...
    def _get_cli_version(self):
        return shell_command_to_string(self.codeql_cli +
                                       ["version"])
    ...
    def _resolve_query_server(self):
        help_msg = shell_command_to_string(self.codeql_cli +
                                           ["excute", "--help"])
    ...
    def _resolve_library_paths(self, query_path):
        ...
        return json.loads(shell_command_to_string(
            self.codeql_cli + args))

    def _resolve_qlpack_paths(self, query_dir):
        return json.loads(shell_command_to_string(
            self.codeql_cli +
            ["resolve", "qlpacks",
             "-v", "--log-to-stderr", "--format=json",
             f"--search-path={query_dir}"]))
...
def get_query_position(query_path: str | Path, target: str):
    ...
    for i, line in enumerate(lines):
        ...
        pattern = (rf"\b({re.escape(target)})\s*\(" if not target[0].isupper()
                   else rf"\bclass\s+({re.escape(target)})\b")
